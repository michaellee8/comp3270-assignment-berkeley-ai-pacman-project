...
def astar_search(problem: Problem):
    ...
    heapq.heappush(pq, (problem.heuristic[problem.start_state], [problem.start_state], 0))
    while pq:
        current_cost, current_path, current_path_cost = heapq.heappop(pq)
        ...
        for next_node, next_path_cost in problem.state_transitions[current_path[-1]].items():
            heapq.heappush(pq, (
                problem.heuristic[next_node] + current_path_cost + next_path_cost, current_path + [next_node],
                current_path_cost + next_path_cost))
